...
def get_featurs(model, test_list, batch_size=10):
    ...
    for i, img_path in enumerate(test_list):
        ...
        if images.shape[0] % batch_size == 0 or i == len(test_list) - 1:
            ...
            fe_1 = output[::2] #odd feature (30,512) batch 60
            fe_2 = output[1::2] #even feature  (30,512)
...
def test_performance(fe_dict, pair_list):
    ...
    for pair in pairs:
        splits = pair.split()
        fe_1 = fe_dict[splits[0]] # 1024 feature
        fe_2 = fe_dict[splits[1]] # 1024 feature
        label = int(splits[2])
        sim = cosin_metric(fe_1, fe_2) # feature angle.. cosine...
...
def lfw_test(model, img_paths, identity_list, compair_list, batch_size):
    ...
    fe_dict = get_feature_dict(identity_list, features) #7701 number
...
if __name__ == '__main__':
    ...
    if opt.backbone == 'resnet18_softmax':
        model = resnet_face18_softmax(opt.use_se)
